...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (fileInput && preview) {
        fileInput.addEventListener('change', function() {
            ...
            cancelBtn.title = 'إلغاء اختيار الملف';
            ...
            if (["jpg","jpeg","png","gif","webp","bmp"].includes(ext)) {
                ...
                img.className = 'rounded border border-white/10 mb-1';
                ...
                video.className = 'rounded border border-white/10 mb-1';
    ...
    try {
        ...
        .then(data => {
            ...
            if (data && data.success) {
                try { form.reset(); if (fileInput) fileInput.value = ''; } catch(e){}
                fetchMessages();
                if (data.file) {
...
function loadActiveDeal(conversationId) {
    ...
    fetch(`get_active_deal.php?conversation_id=${conversationId}`, { credentials: 'include' })
        .then(response => response.json())
        .then(data => {
            const dealSection = document.getElementById('deal-status-section');
            if (data.success && data.deal) {
